...
library(RODBC)
library(stringr)
dbhandle_CIP <- odbcDriverConnect('driver={SQL Server};server=SQLAAG02;database=CIP;
uid=VEN\\coxjil;pwd=x;trusted_connection=yes')

#call stored proc in sql
...
why_left<- Word_by_Word[! (Word_by_Word$word %in% c("driver", "pro", "drivers", "driving", "drive", "3", "company")),]
...
#sort time at particular company into time periods
...
#do the same process for why they liked about the company
#remove stop words
...
#create a vector of words that are not useful in the count
#remove our stop words
whatsgood<- Word_by_Word2[! (Word_by_Word2$word %in% c("driver", "pro", "drivers", "driving", "drive", "3", "company")),]
