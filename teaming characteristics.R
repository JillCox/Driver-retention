...
setwd("//ctg-filesvr/cvenusers$/coxjil/My Documents/Jill Test/Retention/")
# getwd()
...
secrets_for_TMW_sqlquery <- sprintf("driver={SQL Server};server=CTG-SQLAAG02;database=TMW_LIVE;uid=%s;pwd=%s", secrets$username, secrets$password)
dbhandle_TMW_LIVE <- odbcDriverConnect(secrets_for_TMW_sqlquery)

secrets_for_Omnitracs <- sprintf("driver={SQL Server};server=CTG-SQLAAG02;database=Omnitracs;uid=%s;pwd=%s", secrets$username, secrets$password)
dbhandle_Omnitracs <- odbcDriverConnect(secrets_for_Omnitracs)

secrets_for_STRACS <- sprintf("driver={SQL Server};server=CTG-SQLAAG02;database=STRACS;uid=%s;pwd=%s", secrets$username, secrets$password)
dbhandle_STRACS <- odbcDriverConnect(secrets_for_STRACS)
...
string_for_sqlquery3 <- sprintf("select ord_hdrnumber, lgh_number, lgh_driver1, lgh_driver2, lgh_startdate, lgh_enddate,  
                                ...
                                from TMW_LIVE.dbo.legheader with (nolock) 
...
mpp_hist_df <- sqlQuery(dbhandle_TMW_LIVE, (string_for_sqlquery3), as.is = TRUE)
...
mpp_hist_df$lgh_enddate <- as.POSIXct(mpp_hist_df$lgh_enddate, origin='1970-01-01', tz="UTC") # https://stackoverflow.com/questions/30038701/r-as-posixct-dropping-hours-minutes-and-seconds
...
mpp_hist_df$lgh_class1 <- as.character(trimws(mpp_hist_df$lgh_class1)) # starting as character so that I can use substring to replace improper NAs about 15 lines below. Willc onvert to factor after
...
city_df <- sqlQuery(dbhandle_TMW_LIVE, "select cty_code, cty_zip, cty_GMTDelta from TMW_LIVE.dbo.city with (nolock) ")
...
mpp_hist_df$lgh_startdateGMT <- as.POSIXct((mpp_hist_df$lgh_startdate + (mpp_hist_df$lgh_start_cty_GMTDelta*60*60)), origin="1970-01-01", tz="UTC") 
mpp_hist_df$lgh_enddateGMT <- as.POSIXct((mpp_hist_df$lgh_enddate + (mpp_hist_df$lgh_end_cty_GMTDelta*60*60)), origin="1970-01-01", tz="UTC")  

# for cut off dates need east coast time
mpp_hist_df$lgh_startdateEST <- as.POSIXct((mpp_hist_df$lgh_startdate - (4*60*60) + (mpp_hist_df$lgh_start_cty_GMTDelta*60*60)), origin="1970-01-01", tz="UTC") 
mpp_hist_df$lgh_enddateEST <- as.POSIXct((mpp_hist_df$lgh_enddate - (4*60*60) + (mpp_hist_df$lgh_end_cty_GMTDelta*60*60)), origin="1970-01-01", tz="UTC") 

#leg in hours below
# mpp_hist_df$lgh_hours <- as.numeric((difftime(mpp_hist_df$lgh_enddateGMT, mpp_hist_df$lgh_startdateGMT, tz="UTC", units = "mins"))/(60))

mpp_hist_df <- mpp_hist_df[,c("ord_hdrnumber", "lgh_number",  "lgh_driver1", "lgh_driver2", "lgh_startdateEST", "lgh_enddateEST", "lgh_class1", 
                              "trc_division")] 
...
# note all done form lgh_driver1's perspective - note columns were switched to make sure every driver on every leg for the time period analyzed was classified as lgh_driver1
...
#find cost of switching partners
##################################
# add a weekly variable - Using the function strftime passing the argument %V to obtain the week of the year as decimal number (01â???"53) as defined in ISO 8601.
activity_summary_post_mpp2$yyww_identifier <- paste((substr(format(activity_summary_post_mpp2$StartDate, "%Y"),3,4)), (strftime(activity_summary_post_mpp2$StartDate, format = "%V")), sep = "_")
